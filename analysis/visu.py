...
def plot_tier_icons(df):
    ...
    row_height, gap_height = 1.0, 0.02
    label_width, vertical_strip_width = 1.0, 0.02
    ...
    for tier in tier_order:
        ...
        for god in subset["God"]:
            image_path = next((f"god_icons/{god}{ext}" for ext in (".png", ".jpg", ".jpeg") if os.path.exists(f"god_icons/{god}{ext}")), None)
            if not image_path:
                print(f"Missing icon for {god}")
                continue
            ax.imshow(mpimg.imread(image_path), extent=(x_current, x_current + row_height, y_bottom, y_top), aspect="auto")
            x_current += row_height

        y_current = y_bottom - gap_height if tier != tier_order[-1] else y_bottom
        if tier != tier_order[-1]:
            ax.add_patch(patches.Rectangle((0, y_current), total_width, gap_height, facecolor="black"))
